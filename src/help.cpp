...
void help()
{
    ...
    cout << "maxwell-integrate-points=[>0 =50] - число точек интегрирование маквеловского распределения при усредении\n";
...
void helpTime()
{
    ...
    "   time ptNew ratio pt1 pt2 coeff - момент времени задается долей между задаными моментами времени: coeff*t(pt1)+(1-coeff)*t(pt2)\n" <<
...
void helpMesh()
{
    ...
    "         флаг [exp] - если флаг установлен отношение размеров соседних интервалов постояно, иначе постояна их разность\n" <<
    "   E-axis end - завершить задание оси y\n" <<
    "   Mu-axis - ось x\n" <<
    "      array - массив границ" <<
    "         band - выбор полосы\n" <<
    "            y - полоса содержащия значение y\n" <<
    "            index - номер полосы\n" <<
    "         n= - количество границ\n" <<
    "         points - интервалы\n" <<
    "      quad - неравномерное разибение полосы\n" <<
    "         bottom - нижние основание трапеции лижащие на y=const\n"
    "            band - выбор полосы\n" <<
    "               y - полоса содержащия значение y\n" <<
    "               index - номер полосы\n" <<
    "            min - минимальная граница\n" <<
    "            max - максимальная граница\n" <<
    "         top - верхние основание трапеции лижащие на y=const\n"
    "            band - выбор полосы\n" <<
    "               y - полоса содержащия значение y\n" <<
    "               index - номер полосы\n" <<
    "            min - минимальная граница\n" <<
...
void helpSource()
{
    ...
    cout <<
    "    K-sphere - сфера в фазовом прострастве\n" <<
    "        ion - номер типа иона начиная с нуля\n" <<
    "        r - положение по r-axis\n"
    "            [1] value - значения радиуса ячейки\n" <<
    "            [2] index - номер ячейки начиная с нуля\n" <<
    ...
    "        I [lin|log] N - число точек по времени (lin - линейная аппроксицмация, log - логарифмическая апроксимация, по умолчанию нет аппроксимации)\n" <<
    "            pt value - указатель на момент времени значение, интесивность источника в экв. А. [>=0]\n";

    cout <<
    "    K-theta - кольцо в фазовом прострастве\n" <<
    "    K-theta-positive - кольцо фазовом прострасве v||>0\n" <<
    "    K-theta-negative - кольцо фазовом прострасве v||<0\n" <<
    "        ion - номер типа иона начиная с нуля\n" <<
    "        r - положение по r-axis\n"
    "            [1] value - значения радиуса ячейки\n" <<
    "            [2] index - номер ячейки начиная с нуля\n" <<
    ...
    "            [2] index - номер ячейки начиная с нуля\n" <<
    "        K - кинетическая энергия [>=0]\n" <<
    "        theta - питч угол градцсф [>=0 <=90]\n" <<
    "        I [lin|log] N - число точек по времени (lin - линейная аппроксицмация, log - логарифмическая апроксимация, по умолчанию нет аппроксимации)\n" <<
    "            pt value - указатель на момент времени значение, интесивность источника в экв. А. [>=0]\n";

    cout <<
    "    K-maxwell - источник с максвеловским распределением\n" <<
    "        ion - номер типа иона начиная с нуля\n" <<
    "        r - положение по r-axis\n"
    "            [1] value - значения радиуса ячейки\n" <<
    "            [2] index - номер ячейки начиная с нуля\n" <<
...
void helpFStep()
{
    ...
    "   epsilon [lin|log] [=1e-10] N - число точек по времени (lin - линейная аппроксицмация, log - логарифмическая апроксимация, по умолчанию нет аппроксимации)\n" <<
    "       pt value - указатель на момент времени, значение точности рещения СЛАУ"
    ...
    "       tol - порог по отбрасованию новых ILUT элементов\n" <<
    "       shift [=0.] - добавить к диагональным элементам матрицы сдвиг\n" <<
    "       coeff [=1.] - маштабировать диагональные элементы матрицы\n" <<
    "       flag [fast] - если флаг установлен отбрасование элементов по порогу происходит при посчете строки, флаг сброшен отрасование после обработки всей строки\n" <<
    "       flag [queue] - если флаг установлен новые элементы меньше максимальных p отбрасываются сразу, если флаг сброшен отбрасование после обработки строки\n" <<
    "   ILUT end\n" <<
    "   ILUK\n" <<
    ...
    "       coeff [=1.] - маштабировать диагональные элементы матрицы\n" <<
    "   ILUK end\n" <<
    "f-step end - завершить настройку решателся\n";
}